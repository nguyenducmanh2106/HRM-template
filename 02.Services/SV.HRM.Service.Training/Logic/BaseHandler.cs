...
namespace SV.HRM.Service.Training
{
    ...
    public class BaseHandler : IBaseHandler
    {
        ...
        public async Task<Response<T>> CreateObject<T>(string layout, T entityGeneric)
        {
            try
            {
                ...
                var outputResult = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<T>(sqlQuery, objSave, null, CommandType.StoredProcedure);


                if (outputResult != null) //lưu thành công
                {
                    return new Response<T>(Constant.ErrorCode.SUCCESS_CODE, Constant.ErrorCode.SUCCESS_MESS, outputResult);
                }
                else //trường hợp còn lại cụ thể là store proc trả ra = 0 là trường hợp exception lỗi của SQL
                {
                    return new Response<T>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, outputResult);
                }
        ...
        public async Task<Response<T>> UpdateObject<T>(string layout, int id, T entityGeneric)
        {
            try
            {
                var jsonString = JsonConvert.SerializeObject(entityGeneric);
        ...
        /// <param name="entityGeneric"></param>
        /// <returns></returns>
        public async Task<Response<List<T>>> GetFilterCategory<T>(EntityGeneric entityGeneric)
        {
            List<T> listData = new List<T>();
            int totalCount = 0;
            try
