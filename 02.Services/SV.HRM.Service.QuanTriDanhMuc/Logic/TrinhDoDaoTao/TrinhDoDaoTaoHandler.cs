...
namespace SV.HRM.Service.QuanTriDanhMuc
{
    public class TrinhDoDaoTaoHandler : ITrinhDoDaoTaoHandler
    {
        ...
        public async Task<Response<bool>> Create(TrinhDoDaoTaoCreateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(TrinhDoDaoTao)}.json", nameof(TrinhDoDaoTao), "Proc_Create");
                DynamicParameters param = new DynamicParameters();
                param.Add("@TrinhDoDaoTaoCode", model.TrinhDoDaoTaoCode);
                param.Add("@TrinhDoDaoTaoName", model.TrinhDoDaoTaoName);
        ...
        {
            TrinhDoDaoTaoModel result;
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(TrinhDoDaoTao)}.json", nameof(TrinhDoDaoTao), "FindById");
                result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<TrinhDoDaoTaoModel>(sqlQuery, new { TrinhDoDaoTaoID = recordID });
                if (result != null)
                {
                    return new Response<TrinhDoDaoTaoModel>(Constant.ErrorCode.SUCCESS_CODE, Constant.ErrorCode.SUCCESS_MESS, result);
                }
                return new Response<TrinhDoDaoTaoModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, result);

            }
            catch (Exception ex)
            {
                logger.Error($"[ERROR]: {ex}");
                return new Response<TrinhDoDaoTaoModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, null);
            }
        }
        ...
        public async Task<Response<bool>> Update(int id, TrinhDoDaoTaoUpdateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(TrinhDoDaoTao)}.json", nameof(TrinhDoDaoTao), "Proc_Update");
                DynamicParameters param = new DynamicParameters();
                param.Add("@TrinhDoDaoTaoID", id);
                param.Add("@TrinhDoDaoTaoCode", model.TrinhDoDaoTaoCode);
                param.Add("@TrinhDoDaoTaoName", model.TrinhDoDaoTaoName);
                param.Add("@Status", model.Status);
                param.Add("@SoThuTu", model.SoThuTu);
            ...
            try
            {
                ...
                if (dataRecords.Count() > 0 && dataTables.Count() > 0)
                {
                    ...
                    foreach (var item in dataTables)
                    {
                        ...
                        var result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<TrinhDoDaoTao>(sql: queryFormat, new { @recordIDs = dataRecords });
                        if (result != null)
                        {
                            checkUse = true;
                            break;
                ...
                if (!checkUse)
                {
                    ...
                    return await _baseHandler.DeleteManyCheckUseRecord<TrinhDoDaoTao>(data);
                }
                else
                {
