...
{
    public class DiplomaHandler : IDiplomaHandler
    {
        private static Logger logger = LogManager.GetCurrentClassLogger();
        private readonly IDapperUnitOfWork _dapperUnitOfWork;
        private readonly IBaseHandler _baseHandler;
        public DiplomaHandler(IDapperUnitOfWork dapperUnitOfWork,
        ...
        public async Task<Response<bool>> Create(DiplomaCreateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(Diploma)}.json", nameof(Diploma), "Proc_Create");
                DynamicParameters param = new DynamicParameters();
                param.Add("@DiplomaCode", model.DiplomaCode);
                param.Add("@DiplomaName", model.DiplomaName);
        ...
        public async Task<Response<DiplomaModel>> FindById(int recordID)
        {
            ...
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(Diploma)}.json", nameof(Diploma), "FindById");
                result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<DiplomaModel>(sqlQuery, new { DiplomaID = recordID });
                if (result != null)
                {
                    return new Response<DiplomaModel>(Constant.ErrorCode.SUCCESS_CODE, Constant.ErrorCode.SUCCESS_MESS, result);
                }
                return new Response<DiplomaModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, result);

            }
            catch (Exception ex)
            {
                logger.Error($"[ERROR]: {ex}");
                return new Response<DiplomaModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, null);
            }
        }
        ...
        public async Task<Response<bool>> Update(int id, DiplomaUpdateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(Diploma)}.json", nameof(Diploma), "Proc_Update");
                DynamicParameters param = new DynamicParameters();
                param.Add("@DiplomaID", id);
                param.Add("@DiplomaCode", model.DiplomaCode);
                param.Add("@DiplomaName", model.DiplomaName);
                param.Add("@InactiveDate", model.InactiveDate);
                param.Add("@LastUpdatedBy", model.LastUpdatedBy);
        ...
        public async Task<Response<bool>> FindIdInUse(List<object> recordID)
        {
            try
            {
                ...
                string layout = typeof(Diploma).Name;
                ...
                if (dataRecords > 0 && dataTable.Count() > 0)
                {
                    foreach (var tbl in dataTable)
                    {
                        if (!layout.Equals(tbl))
                        {
                            ...
                            string sqlQuery2 = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(Diploma)}.json", nameof(Diploma), "CountByIdRecord");
                            string queryFormat = string.Format(sqlQuery2, layoutChild, nameof(Diploma) + "ID");
                            var result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<int>(sql: queryFormat, new { @DiplomaID = dataRecords });
                            if (result > 0)
                            {
                                checkUse = true;
                                break;
            ...
            try
            {
                ...
                if (dataRecords.Count() > 0 && dataTables.Count() > 0)
                {
                    ...
                    foreach (var item in dataTables)
                    {
                        ...
                        var result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<Diploma>(sql: queryFormat, new { @recordIDs = dataRecords });
                        if (result != null)
                        {
                            checkUse = true;
                            break;
                ...
                if (!checkUse)
                {
                    ...
                    return await _baseHandler.DeleteManyCheckUseRecord<Diploma>(data);
                }
                else
                {
