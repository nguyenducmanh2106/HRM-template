...
namespace SV.HRM.Service.QuanTriDanhMuc
{
    public class RewardHandler : IRewardHandler
    {
        ...
        public RewardHandler(IDapperUnitOfWork dapperUnitOfWork,
        ...
        public async Task<Response<bool>> Create(RewardCreateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(Reward)}.json", nameof(Reward), "Proc_Create");
                DynamicParameters param = new DynamicParameters();
                param.Add("@RewardCode", model.RewardCode);
                param.Add("@RewardName", model.RewardName);
        ...
        public async Task<Response<RewardModel>> FindById(int recordID)
        {
            ...
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(Reward)}.json", nameof(Reward), "FindById");
                result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<RewardModel>(sqlQuery, new { RewardID = recordID });
                if (result != null)
                {
                    return new Response<RewardModel>(Constant.ErrorCode.SUCCESS_CODE, Constant.ErrorCode.SUCCESS_MESS, result);
                }
                return new Response<RewardModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, result);

            }
            catch (Exception ex)
            {
                logger.Error($"[ERROR]: {ex}");
                return new Response<RewardModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, null);
            }
        }
        ...
        public async Task<Response<bool>> Update(int id, RewardUpdateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(Reward)}.json", nameof(Reward), "Proc_Update");
                DynamicParameters param = new DynamicParameters();
                param.Add("@RewardID", id);
                param.Add("@RewardCode", model.RewardCode);
                param.Add("@RewardName", model.RewardName);
                param.Add("@Status", model.Status);
                param.Add("@LastUpdatedBy", model.LastUpdatedBy);
            ...
            try
            {
                ...
                if (dataRecords.Count() > 0 && dataTables.Count() > 0)
                {
                    ...
                    foreach (var item in dataTables)
                    {
                        ...
                        var result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<Reward>(sql: queryFormat, new { @recordIDs = dataRecords });
                        if (result != null)
                        {
                            checkUse = true;
                            break;
                ...
                if (!checkUse)
                {
                    ...
                    return await _baseHandler.DeleteManyCheckUseRecord<Reward>(data);
                }
                else
                {
