...
{
    public class PartyTitleHandler : IPartyTitleHandler
    {
        private static Logger logger = LogManager.GetCurrentClassLogger();
        private readonly IDapperUnitOfWork _dapperUnitOfWork;
        private readonly IBaseHandler _baseHandler;

        public PartyTitleHandler(IDapperUnitOfWork dapperUnitOfWork, IBaseHandler baseHandler)
        {
        ...
        public async Task<Response<bool>> Create(PartyTitleCreateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(PartyTitle)}.json", nameof(PartyTitle), "Proc_Create");
                DynamicParameters param = new DynamicParameters();
                param.Add("@PartyTitleCode", model.PartyTitleCode);
                param.Add("@PartyTitleName", model.PartyTitleName);
        ...
        {
            PartyTitleModel result;
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(PartyTitle)}.json", nameof(PartyTitle), "FindById");
                result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<PartyTitleModel>(sqlQuery, new { PartyTitleID = recordID });
                if (result != null)
                {
                    return new Response<PartyTitleModel>(Constant.ErrorCode.SUCCESS_CODE, Constant.ErrorCode.SUCCESS_MESS, result);
                }
                return new Response<PartyTitleModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, result);

            }
            catch (Exception ex)
            {
                logger.Error($"[ERROR]: {ex}");
                return new Response<PartyTitleModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, null);
            }
        }
        ...
        public async Task<Response<bool>> Update(int id, PartyTitleUpdateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(PartyTitle)}.json", nameof(PartyTitle), "Proc_Update");
                DynamicParameters param = new DynamicParameters();
                param.Add("@PartyTitleID", id);
                param.Add("@PartyTitleCode", model.PartyTitleCode);
                param.Add("@PartyTitleName", model.PartyTitleName);
                param.Add("@LastUpdatedBy", model.LastUpdatedBy);
                param.Add("@LastUpdatedDate", DateTime.Now);
            ...
            try
            {
                ...
                if (dataRecords.Count() > 0 && dataTables.Count() > 0)
                {
                    ...
                    foreach (var item in dataTables)
                    {
                        ...
                        var result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<PartyTitle>(sql: queryFormat, new { @recordIDs = dataRecords });
                        if (result != null)
                        {
                            checkUse = true;
                            break;
                ...
                if (!checkUse)
                {
                    ...
                    return await _baseHandler.DeleteManyCheckUseRecord<PartyTitle>(data);
                }
                else
                {
