...
{
    public class SpecialityHandler : ISpecialityHandler
    {
        private static Logger logger = LogManager.GetCurrentClassLogger();
        private readonly IDapperUnitOfWork _dapperUnitOfWork;
        private readonly IBaseHandler _baseHandler;

        public SpecialityHandler(IDapperUnitOfWork dapperUnitOfWork,
        ...
        public async Task<Response<bool>> Create(SpecialityCreateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(Speciality)}.json", nameof(Speciality), "Proc_Create");
                DynamicParameters param = new DynamicParameters();
                param.Add("@SpecialityCode", model.SpecialityCode);
                param.Add("@SpecialityName", model.SpecialityName);
        ...
        public async Task<Response<SpecialityModel>> FindById(int recordID)
        {
            ...
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(Speciality)}.json", nameof(Speciality), "FindById");
                result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<SpecialityModel>(sqlQuery, new { SpecialityID = recordID });
                if (result != null)
                {
                    return new Response<SpecialityModel>(Constant.ErrorCode.SUCCESS_CODE, Constant.ErrorCode.SUCCESS_MESS, result);
                }
                return new Response<SpecialityModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, result);

            }
            catch (Exception ex)
            {
                logger.Error($"[ERROR]: {ex}");
                return new Response<SpecialityModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, null);
            }
        }
        /// <summary>
        /// cập nhật hệ đào tạo
        ...
        public async Task<Response<bool>> Update(int id, SpecialityUpdateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(Speciality)}.json", nameof(Speciality), "Proc_Update");
                DynamicParameters param = new DynamicParameters();
                param.Add("@SpecialityID", id);
                param.Add("@SpecialityCode", model.SpecialityCode);
                param.Add("@SpecialityName", model.SpecialityName);
                param.Add("@InactiveDate", model.InactiveDate);
                param.Add("@LastUpdatedBy", model.LastUpdatedBy);
        ...
        public async Task<Response<bool>> FindIdInUse(List<object> recordID)
        {
            try
            {
                ...
                string layout = typeof(Speciality).Name;
                ...
                if (dataRecords > 0 && dataTable.Count() > 0)
                {
                    foreach (var tbl in dataTable)
                    {
                        if (!layout.Equals(tbl))
                        {
                            ...
                            string sqlQuery2 = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(Speciality)}.json", nameof(Speciality), "CountByIdRecord");
                            string queryFormat = string.Format(sqlQuery2, layoutChild, nameof(Speciality) + "ID");
                            var result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<int>(sql: queryFormat, new { @SpecialityID = dataRecords });
                            if (result > 0)
                            {
                                checkUse = true;
                                break;
            ...
            try
            {
                ...
                if (dataRecords.Count() > 0 && dataTables.Count() > 0)
                {
                    ...
                    foreach (var item in dataTables)
                    {
                        ...
                        var result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<Speciality>(sql: queryFormat, new { @recordIDs = dataRecords });
                        if (result != null)
                        {
                            checkUse = true;
                            break;
                ...
                if (!checkUse)
                {
                    ...
                    return await _baseHandler.DeleteManyCheckUseRecord<Speciality>(data);
                }
                else
                {
