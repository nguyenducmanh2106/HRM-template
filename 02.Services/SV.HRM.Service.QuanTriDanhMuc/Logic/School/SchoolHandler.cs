...
namespace SV.HRM.Service.QuanTriDanhMuc
{
    public class SchoolHandler : ISchoolHandler
    {
        ...
        public SchoolHandler(IDapperUnitOfWork dapperUnitOfWork,
        ...
        public async Task<Response<bool>> Create(SchoolCreateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(School)}.json", nameof(School), "Proc_Create");
                DynamicParameters param = new DynamicParameters();
                param.Add("@SchoolCode", model.SchoolCode);
                param.Add("@SchoolName", model.SchoolName);
        ...
        public async Task<Response<SchoolModel>> FindById(int recordID)
        {
            ...
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(School)}.json", nameof(School), "FindById");
                result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<SchoolModel>(sqlQuery, new { SchoolID = recordID });
                if (result != null)
                {
                    return new Response<SchoolModel>(Constant.ErrorCode.SUCCESS_CODE, Constant.ErrorCode.SUCCESS_MESS, result);
                }
                return new Response<SchoolModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, result);

            }
            catch (Exception ex)
            {
                logger.Error($"[ERROR]: {ex}");
                return new Response<SchoolModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, null);
            }
        }
        /// <summary>
        ...
        public async Task<Response<bool>> Update(int id, SchoolUpdateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(School)}.json", nameof(School), "Proc_Update");
                DynamicParameters param = new DynamicParameters();
                param.Add("@SchoolID", id);
                param.Add("@SchoolCode", model.SchoolCode);
                param.Add("@SchoolName", model.SchoolName);
                param.Add("@Addr", model.Addr);
                param.Add("@City", model.City);
            ...
            try
            {
                ...
                if (dataRecords > 0 && dataTable.Count > 0)
                {
                    foreach (var tbl in dataTable)
                    {
                        ...
                        string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(School)}.json", nameof(School), "CountByIdRecord");
                        string queryFormat = string.Format(sqlQuery, layoutChild, nameof(School) + "ID");
                        var result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<int>(sql: queryFormat, new { @SchoolID = dataRecords });
                        if (result > 0)
                        {
                            checkUse = true;
                            break;
            ...
            try
            {
                ...
                if (dataRecords.Count() > 0 && dataTables.Count() > 0)
                {
                    ...
                    foreach (var item in dataTables)
                    {
                        ...
                        var result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<School>(sql: queryFormat, new { @recordIDs = dataRecords });
                        if (result != null)
                        {
                            checkUse = true;
                            break;
                ...
                if (!checkUse)
                {
                    ...
                    return await _baseHandler.DeleteManyCheckUseRecord<School>(data);
                }
                else
                {
