...
{
    public class DisciplineTypeHandler : IDisciplineTypeHandler
    {
        private static Logger logger = LogManager.GetCurrentClassLogger();
        private readonly IDapperUnitOfWork _dapperUnitOfWork;
        private readonly IBaseHandler _baseHandler;
        public DisciplineTypeHandler(IDapperUnitOfWork dapperUnitOfWork,
        ...
        public async Task<Response<bool>> Create(DisciplineTypeCreateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(DisciplineType)}.json", nameof(DisciplineType), "Proc_Create");
                DynamicParameters param = new DynamicParameters();
                param.Add("@DisciplineTypeCode", model.DisciplineTypeCode);
                param.Add("@Description1", model.Description1);
        ...
        {
            DisciplineTypeModel result;
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(DisciplineType)}.json", nameof(DisciplineType), "FindById");
                result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<DisciplineTypeModel>(sqlQuery, new { DisciplineTypeID = recordID });
                if (result != null)
                {
                    return new Response<DisciplineTypeModel>(Constant.ErrorCode.SUCCESS_CODE, Constant.ErrorCode.SUCCESS_MESS, result);
                }
                return new Response<DisciplineTypeModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, result);

            }
            catch (Exception ex)
            {
                logger.Error($"[ERROR]: {ex}");
                return new Response<DisciplineTypeModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, null);
            }
        }
        ...
        public async Task<Response<bool>> Update(int id, DisciplineTypeUpdateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(DisciplineType)}.json", nameof(DisciplineType), "Proc_Update");
                DynamicParameters param = new DynamicParameters();
                param.Add("@DisciplineTypeID", id);
                param.Add("@DisciplineTypeCode", model.DisciplineTypeCode);
                param.Add("@Description1", model.Description1);
                param.Add("@LastUpdatedBy", model.LastUpdatedBy);
                param.Add("@LastUpdatedDate", DateTime.Now);
            ...
            try
            {
                ...
                if (dataRecords.Count() > 0 && dataTables.Count() > 0)
                {
                    ...
                    foreach (var item in dataTables)
                    {
                        ...
                        var result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<DisciplineType>(sql: queryFormat, new { @recordIDs = dataRecords });
                        if (result != null)
                        {
                            checkUse = true;
                            break;
                ...
                if (!checkUse)
                {
                    ...
                    return await _baseHandler.DeleteManyCheckUseRecord<DisciplineType>(data);
                }
                else
                {
