...
{
    public class ChucVuKiemNhiemHandler : IChucVuKiemNhiemHandler
    {
        private static Logger logger = LogManager.GetCurrentClassLogger();
        private readonly IDapperUnitOfWork _dapperUnitOfWork;
        private readonly IBaseHandler _baseHandler;
        public ChucVuKiemNhiemHandler(IDapperUnitOfWork dapperUnitOfWork,
        ...
        public async Task<Response<bool>> Create(ChucVuKiemNhiemCreateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(ChucVuKiemNhiem)}.json", nameof(ChucVuKiemNhiem), "Proc_Create");
                DynamicParameters param = new DynamicParameters();
                param.Add("@ChucVuKiemNhiemCode", model.ChucVuKiemNhiemCode);
                param.Add("@ChucVuKiemNhiemName", model.ChucVuKiemNhiemName);
        ...
        {
            ChucVuKiemNhiemModel result;
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(ChucVuKiemNhiem)}.json", nameof(ChucVuKiemNhiem), "FindById");
                result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<ChucVuKiemNhiemModel>(sqlQuery, new { ChucVuKiemNhiemID = recordID });
                if (result != null)
                {
                    return new Response<ChucVuKiemNhiemModel>(Constant.ErrorCode.SUCCESS_CODE, Constant.ErrorCode.SUCCESS_MESS, result);
                }
                return new Response<ChucVuKiemNhiemModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, result);

            }
            catch (Exception ex)
            {
                logger.Error($"[ERROR]: {ex}");
                return new Response<ChucVuKiemNhiemModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, null);
            }
        }
        ...
        public async Task<Response<bool>> Update(int id, ChucVuKiemNhiemUpdateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(ChucVuKiemNhiem)}.json", nameof(ChucVuKiemNhiem), "Proc_Update");
                DynamicParameters param = new DynamicParameters();
                param.Add("@ChucVuKiemNhiemID", id);
                param.Add("@ChucVuKiemNhiemCode", model.ChucVuKiemNhiemCode);
                param.Add("@ChucVuKiemNhiemName", model.ChucVuKiemNhiemName);
                param.Add("@GhiChu", model.GhiChu);
                param.Add("@Status", model.Status);
            ...
            try
            {
                ...
                if (dataRecords.Count() > 0 && dataTables.Count() > 0)
                {
                    ...
                    foreach (var item in dataTables)
                    {
                        ...
                        var result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<ChucVuKiemNhiem>(sql: queryFormat, new { @recordIDs = dataRecords });
                        if (result != null)
                        {
                            checkUse = true;
                            break;
                ...
                if (!checkUse)
                {
                    ...
                    return await _baseHandler.DeleteManyCheckUseRecord<ChucVuKiemNhiem>(data);
                }
                else
                {
