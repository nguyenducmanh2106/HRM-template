...
{
    public class FamilyRelationShipHandler : IFamilyRelationShipHandler
    {
        private static Logger logger = LogManager.GetCurrentClassLogger();
        private readonly IDapperUnitOfWork _dapperUnitOfWork;
        private readonly IBaseHandler _baseHandler;

        public FamilyRelationShipHandler(IDapperUnitOfWork dapperUnitOfWork, IBaseHandler baseHandler)
        {
        ...
        public async Task<Response<bool>> Create(FamilyRelationShipCreateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(FamilyRelationShip)}.json", nameof(FamilyRelationShip), "Proc_Create");
                DynamicParameters param = new DynamicParameters();
                param.Add("@FamilyRelationshipCode", model.FamilyRelationshipCode);
                param.Add("@FamilyRelationshipName", model.FamilyRelationshipName);
        ...
        {
            FamilyRelationShipModel result;
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(FamilyRelationShip)}.json", nameof(FamilyRelationShip), "FindById");
                result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<FamilyRelationShipModel>(sqlQuery, new { FamilyRelationshipID = recordID });
                if (result != null)
                {
                    return new Response<FamilyRelationShipModel>(Constant.ErrorCode.SUCCESS_CODE, Constant.ErrorCode.SUCCESS_MESS, result);
                }
                return new Response<FamilyRelationShipModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, result);

            }
            catch (Exception ex)
            {
                logger.Error($"[ERROR]: {ex}");
                return new Response<FamilyRelationShipModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, null);
            }
        }
        ...
        public async Task<Response<bool>> Update(int id, FamilyRelationShipUpdateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(FamilyRelationShip)}.json", nameof(FamilyRelationShip), "Proc_Update");
                DynamicParameters param = new DynamicParameters();
                param.Add("@FamilyRelationshipID", id);
                param.Add("@FamilyRelationshipCode", model.FamilyRelationshipCode);
                param.Add("@FamilyRelationshipName", model.FamilyRelationshipName);
                param.Add("@Status", model.Status);
                param.Add("@SoThuTu", model.SoThuTu);
            ...
            try
            {
                ...
                if (dataRecords.Count() > 0 && dataTables.Count() > 0)
                {
                    ...
                    foreach (var item in dataTables)
                    {
                        ...
                        var result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<FamilyRelationShip>(sql: queryFormat, new { @recordIDs = dataRecords });
                        if (result != null)
                        {
                            checkUse = true;
                            break;
                ...
                if (!checkUse)
                {
                    ...
                    return await _baseHandler.DeleteManyCheckUseRecord<FamilyRelationShip>(data);
                }
                else
                {
