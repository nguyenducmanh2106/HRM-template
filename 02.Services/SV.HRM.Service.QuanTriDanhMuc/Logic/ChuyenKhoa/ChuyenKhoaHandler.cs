...
{
    public class ChuyenKhoaHandler : IChuyenKhoaHandler
    {
        private static Logger logger = LogManager.GetCurrentClassLogger();
        private readonly IDapperUnitOfWork _dapperUnitOfWork;
        private readonly IBaseHandler _baseHandler;
        public ChuyenKhoaHandler(IDapperUnitOfWork dapperUnitOfWork,
        ...
        public async Task<Response<bool>> Create(ChuyenKhoaCreateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(ChuyenKhoa)}.json", nameof(ChuyenKhoa), "Proc_Create");
                DynamicParameters param = new DynamicParameters();
                param.Add("@MaChuyenKhoa", model.MaChuyenKhoa);
                param.Add("@TenChuyenKhoa", model.TenChuyenKhoa);
        ...
        {
            ChuyenKhoaModel result;
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(ChuyenKhoa)}.json", nameof(ChuyenKhoa), "FindById");
                result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<ChuyenKhoaModel>(sqlQuery, new { ChuyenKhoaID = recordID });
                if (result != null)
                {
                    return new Response<ChuyenKhoaModel>(Constant.ErrorCode.SUCCESS_CODE, Constant.ErrorCode.SUCCESS_MESS, result);
                }
                return new Response<ChuyenKhoaModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, result);

            }
            catch (Exception ex)
            {
                logger.Error($"[ERROR]: {ex}");
                return new Response<ChuyenKhoaModel>(Constant.ErrorCode.FAIL_CODE, Constant.ErrorCode.FAIL_MESS, null);
            }
        }
        ...
        public async Task<Response<bool>> Update(int id, ChuyenKhoaUpdateModel model)
        {
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/{nameof(ChuyenKhoa)}.json", nameof(ChuyenKhoa), "Proc_Update");
                DynamicParameters param = new DynamicParameters();
                param.Add("@ChuyenKhoaID", id);
                param.Add("@MaChuyenKhoa", model.MaChuyenKhoa);
                param.Add("@TenChuyenKhoa", model.TenChuyenKhoa);
                param.Add("@GhiChu", model.GhiChu);
                param.Add("@Status", model.Status);
            ...
            try
            {
                ...
                if (dataRecords.Count() > 0 && dataTables.Count() > 0)
                {
                    ...
                    foreach (var item in dataTables)
                    {
                        ...
                        var result = await _dapperUnitOfWork.GetRepository().QuerySingleOrDefaultAsync<ChuyenKhoa>(sql: queryFormat, new { @recordIDs = dataRecords });
                        if (result != null)
                        {
                            checkUse = true;
                            break;
                ...
                if (!checkUse)
                {
                    ...
                    return await _baseHandler.DeleteManyCheckUseRecord<ChuyenKhoa>(data);
                }
                else
                {
