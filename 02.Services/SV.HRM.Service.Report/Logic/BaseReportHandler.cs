...
{
    public class BaseReportHandler : IBaseReportHandler
    {
        ...
        public BaseReportHandler(IMapper mapper, ICached cached, IDapperUnitOfWork dapperUnitOfWork, IConfiguration configuration,
        ...
        /// <returns></returns>
        public async Task<Response<List<T>>> GetFilter<T>(EntityGeneric entityGeneric, bool? hasSummaryData = false)
        {
            List<T> listData = new List<T>();
            int totalCount = 0;
            try
            {
                string sqlQuery = JSONObject.GetQueryFromJSON($"SqlCommand/Report.json", "Report", entityGeneric.LayoutCode);
                ...
                {
                    object summaryData = hasSummaryData == true && objReturn[2] != null ? objReturn[2] : null;
                    ...
                    return new Response<List<T>>(Constant.ErrorCode.SUCCESS_CODE, Constant.ErrorCode.SUCCESS_MESS, listData, listData.Count(), totalCount, totalPage, entityGeneric.PageIndex, entityGeneric.PageSize, null, summaryData);
                }
                else
                {
            ...
            try
            {
                ...
                using (IDbConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    using (var unitOfwork = new DapperUnitOfWork(connection))
                    {
                        ...
                        var ret = dal.Query<OrganizationModel>("SELECT OrganizationID,OrganizationCode,OrganizationName FROM Organizations WHERE Status = @status;", new { status = Constant.StatusRecord.ACTIVE });
                        return ret.ToList();
                    }
                }
            }
